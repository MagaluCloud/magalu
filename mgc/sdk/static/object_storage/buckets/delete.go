...
type deleteParams struct {
	...
	RecursiveAsync bool              `json:"recursive-async" jsonschema:"description=If set to true and Recursive is also set to true, the objects are deleted asynchronously"`
}
...
func deleteBucket(ctx context.Context, params deleteParams, cfg common.Config) (core.Value, error) {
	...
	if params.Recursive && params.RecursiveAsync {
		return nil, errors.New("bucket deleta can either be '--recursive' OR '--recursive-async'. Not both")
	}

	// Recursively delete objects synchronusly
	if params.Recursive {
	...
	err := common.DeleteBucket(ctx, common.DeleteBucketParams{Destination: dst, RecursiveAsync: params.RecursiveAsync}, cfg)
	if err != nil {
		return nil, err
	}
