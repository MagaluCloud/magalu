...
```bash
Usage:
  ./mgc load-balancer network-certificates replace [load-balancer-id] [tls-certificate-id] [flags]
```
...
- --load-balancer-id string       load_balancer_id: ID of the load balancer to update the TLS certificate (required)
