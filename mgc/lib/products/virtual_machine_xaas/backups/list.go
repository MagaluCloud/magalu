...
Lists all the virtual machine backups of the current logged-in tenant

	filtered by the project type specified.
...
func (s *service) List(
	...
	exec, ctx, err := mgcHelpers.PrepareExecutor("List", mgcCore.RefPath("/virtual-machine-xaas/backups/list"), s.client, s.ctx)
	if err != nil {
		return
	}
