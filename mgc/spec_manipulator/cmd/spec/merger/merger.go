...
// SpecMerger é responsável por mesclar especificações OpenAPI
type SpecMerger struct {
}

// MergeOptions define as opções para mesclagem de especificações
type MergeOptions struct {
	DowngradeToVersion string       // Versão para downgrade se necessário
	Servers            []*v3.Server // Servidores personalizados para adicionar
}

// NewSpecMerger cria uma nova instância do mesclador de especificações
func NewSpecMerger() *SpecMerger {
	return &SpecMerger{}
}

// MergeSpecs mescla duas especificações OpenAPI e salva o resultado
func (m *SpecMerger) MergeSpecs(specAPath, specBPath, outputPath string, options *MergeOptions) error {
	...
	// Carregar a segunda especificação (opcional)
...
// performMerge realiza a mesclagem de duas especificações OpenAPI
...
// mergePaths mescla os caminhos de duas especificações
func (m *SpecMerger) mergePaths(mergedSpecA *libopenapi.DocumentModel[v3.Document], specModelB *libopenapi.DocumentModel[v3.Document]) error {
	...
	for path := specModelB.Model.Paths.PathItems.Oldest(); path != nil; path = path.Next() {
		...
		// Adicionar tags dos caminhos às tags principais
...
// mergeOperation mescla duas operações
...
// mergeParameter mescla dois parâmetros
...
// addPathTagsToMainTags adiciona tags usadas em caminhos à lista principal de tags
...
// addTagIfNotExists adiciona uma tag à lista principal se ela não existir
...
// mergeSchemas mescla os esquemas de componentes
func (m *SpecMerger) mergeSchemas(mergedSpecA *libopenapi.DocumentModel[v3.Document], specModelB *libopenapi.DocumentModel[v3.Document]) error {
	...
	// Verifica se Components existe no spec A
...
// mergeTags mescla as tags de duas especificações
...
// saveSpec salva a especificação mesclada em um arquivo
