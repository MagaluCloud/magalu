...
func mergeSpecsMain(options MergeSpecs) {
	...
	if options.specB != "" {
		...
		fileBytes, err = os.ReadFile(file)
		if err != nil {
			fmt.Printf("Erro ao carregar a customização: %v\n", err)
		} else {
...
func mergeSpecs(specA, specB libopenapi.Document, options MergeSpecs) libopenapi.Document {
	...
	mergedSpecA.Model.Version = "3.0.3"
	...
	mergedSpecA.Model.Servers = []*v3.Server{server}
...
func MergeSpecsCmd() *cobra.Command {
	...
	cmd.Flags().StringVarP(&options.specA, "speca", "a", "", "Path to the first OpenAPI specification file")
